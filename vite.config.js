...
function CompareHtmlMinifiersPlugin() {
  return {
    name: "compare-html-minifiers",
    apply: "build",
    closeBundle: async () => {
      ...
      // Uncomment to use the smallest as main index.html
      // await fs.copyFile(path.join(distDir, `index.${smallest.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}.html`), indexPath);
    },
  };
}
...
export default defineConfig({
  ...
  plugins: [CompareHtmlMinifiersPlugin()],
});
